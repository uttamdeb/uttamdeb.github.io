...
- Android is the platform where a plain `.vcf` still forces a download-then-tap detour, so `share-cards.js` swaps the href for an `intent://` ACTION_INSERT URL on Chromium Android, which opens the contact editor directly. It carries no photo — that is the trade for skipping the download — and `S.browser_fallback_url` returns anything that cannot handle the intent to the vCard. Firefox Android has no `intent:` support and is deliberately left on the vCard. The markup's href is always the vCard, so this is purely an upgrade and no-JS is unaffected. **This path cannot be exercised in a desktop browser; check it on a real Android handset before relying on it.**
